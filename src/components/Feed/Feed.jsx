    ...
    return (
        <div className="container mx-auto max-w-screen-lg">
            ...
            <div className="col my-4 bg-zinc-200 rounded-lg shadow">
                ...
                {feed.media && <div className='relative w-full h-[30rem] px-8 overflow-hidden cursor-default'>
                    ...
                    {feed.media.map((item, index) => {
                        if (imgFormats?.includes(get_url_extension(item.link))) {
                            return <img className='media w-auto h-full absolute -translate-x-1/2 transition-all ease-in-out duration-700' style={{ left: `${(index - currMed) * 100 + 50}%` }} src={item.link} />
                        }
                        else if (vidFormats?.includes(get_url_extension(item.link))) {
                            return <ReactPlayer url={item.link} className='media h-full absolute -translate-x-1/2 transition-all ease-in-out duration-700' width="100%" height="100%" style={{ left: `${(index - currMed) * 100 + 50}%` }} controls />
                        }
                    })}
                </div>}
